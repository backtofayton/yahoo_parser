...
def yahoo_parser(ticker):    # 347155200
    # Set time to 10 minutes ago to be safe
    current_date = str(date.now().timestamp()-600)[:-7]
    yahoo_link = 'https://query1.finance.yahoo.com/v7/finance/download/%s?period1=1594515200&period2=%s&interval=1d&events=history&includeAdjustedClose=true' % (
        ticker, current_date)
    print(yahoo_link)
    ...
    with open('%s.json' % ticker, 'w') as fd:
        ...
        for chunk in req.iter_content(chunk_size=128):
            if titleParse:
                # Regex catches in the beginning of document for all the value before to set Titles '\n'
            ...
            if len(findDates) > 0:
                ...
                for index in range(len(findDates)):
                    ...
                    print('index = ', index)
                    if findData:
                        parsedData = chunk[findData.start(
                        ):findData.end()].split(b',')
                        ...
                        for titleIndex in range(len(parsedTitles)):
                            ...
                            if (titleIndex == 0):
                                ...
                                # parsed Data doesnt have Date, set it's index to -1
                                chunkObject[findDates[index].decode(encoding)].update(
                                    {parsedTitles[titleIndex].decode(encoding): parsedData[titleIndex-1].decode(encoding)})
                        print(parsedData)
